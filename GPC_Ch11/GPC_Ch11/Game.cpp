...
//  GPC_Ch11
//
//  Created by Inoue Shinichi on 2023/07/01.
//
...
void Game::UpdateGame()
{
    ...
    // Compute delta time
    // What until 16ms has elapsed since last frame.
...
void Game::LoadData()
{
    ...
    // Setup lights
    mRenderer->SetAmbientLight(Vector3(0.2f, 0.2f, 0.2f)); // 環境光
    DirectionalLight& dir = mRenderer->GetDirectionalLight();
    dir.mDirection = Vector3(0.0f, -0.707f, -0.707f); // 平行光
    dir.mDiffuseColor = Vector3(0.78f, 0.88f, 1.0f);  // 拡散反射光成分
    dir.mSpecColor = Vector3(0.8f, 0.8f, 0.8f);       // 鏡面反射光成分
    ...
    a->SetPosition(Vector3(1450.0f, 0.0f, 100.0f)); // 1
    a = new TargetActor(this);
    a->SetPosition(Vector3(1450.0f, 0.0f, 400.0f)); // 2
    a = new TargetActor(this);
    a->SetPosition(Vector3(1450.0f, -500.0f, 200.0f)); // 3
    a = new TargetActor(this);
    a->SetPosition(Vector3(1450.0f, 500.0f, 200.0f)); // 4
    a = new TargetActor(this);
    a->SetPosition(Vector3(0.0f, -1450.0f, 200.0f));
    a->SetRotation(Quaternion(Vector3::UnitZ, Math::PiOver2)); // 5
    a = new TargetActor(this);
    a->SetPosition(Vector3(0.0f, 1450.0f, 200.0f));
    a->SetRotation(Quaternion(Vector3::UnitZ, -Math::PiOver2)); // 6
}
...
void Game::LoadText(const std::string& fileName)
{
         ...
         iter != actions.MemberEnd(); ++iter)
    {
        ...
        {
            mText.emplace(iter->name.GetString(),
