...
LineSegment::LineSegment(const Vector3& start,
                         const Vector3& end)
    : mStart(start)
    , mEnd(end)
{}
...
float LineSegment::MinDistSq(const LineSegment &s1, const LineSegment &s2)
{
    ...
    else // get the closest point on the infinite lines
    {
        sN = (b*e - c*d);
        tN = (a*e - b*d);
...
Capsule::Capsule(const Vector3& start,
                 const Vector3& end,
                 float radius)
    : mSegment(start, end)
    , mRadius(radius)
...
bool ConvexPolygon::Contains(const Vector2& point) const
{
    ...
    // (この方法は精度が悪い)
    // Return true if approximately 2*pi
...
// LineSegment vs Sphere
bool Intersect(const LineSegment& l, const Sphere& s, float& outT)
{
    // 球と線分の2つの交点を求める方程式を解く
    ...
    float discriminant = b*b - 4.0f*a*b; // 判別式
    ...
    else
    {
        ...
        // Compute min and solutions of t
...
// LineSegment vs Plane
bool Intersect(const LineSegment& l, const Plane& p, float& outT)
{
    // First test if there's a solution for t
    float denominator = Vector3::Dot(l.mEnd - l.mStart, p.mNormal);
    if (Math::NearZero(denominator)) // 平面と線分が平行
    {
        // The only way they intersect is if start is a point on the plane
        // (P dot N) == d
    ...
    {
        float numerator = -Vector3::Dot(l.mStart, p.mNormal) - p.mD;
...
// LineSegment vs Plane
bool TestSidePlane(
                   /* LineSegment */float start,
                   /* LineSegment */float end,
                   /* Plane */float negd,
                   /* Plane */const Vector3& normal,
                   /* Visitor */std::vector<std::pair<float, Vector3>>& out
                   )
{
               ...
               Vector3& outNormal)
{
    ...
    std::vector<std::pair<float, Vector3>> tValues; // <t, normal>
    
    /* 線分LとAABBの6面でそれぞれ衝突検知を行う */
    
    /* x directional planes */
    // Test the x plane (min_x)
    TestSidePlane(l.mStart.x, l.mEnd.x,
                  b.mMin.x, Vector3::NegUnitX,
                  tValues);
    // Test the x plane (max_x)
    TestSidePlane(l.mStart.x, l.mEnd.x,
                  b.mMax.x, Vector3::UnitX,
                  tValues);
    
    /* y directional planes */
    // Test the y plane (min_y)
    TestSidePlane(l.mStart.y, l.mEnd.y,
                  b.mMin.y, Vector3::NegUnitY,
                  tValues);
    // Test the y plane (max_y)
    TestSidePlane(l.mStart.y, l.mEnd.y,
                  b.mMax.y, Vector3::UnitY,
                  tValues);
   
    /* z directional planes */
    // Test the z plane (min_z)
    TestSidePlane(l.mStart.z, l.mEnd.z,
                  b.mMin.z, Vector3::NegUnitZ,
                  tValues);
    // Test the z plane (max_z)
    TestSidePlane(l.mStart.z, l.mEnd.z,
                  b.mMax.z, Vector3::UnitZ,
                  tValues);
    
    // Sort the t values in ascending order (昇順)
    std::sort(tValues.begin(),
              tValues.end(),
              [](const std::pair<float, Vector3>& a,
                 const std::pair<float, Vector3>& b) -> bool {
    ...
    for (auto& t : tValues)
    {
        point = l.PointOnSegment(/*min_t*/t.first); // 線分Lのtの内分点の位置(tx,ty,tz)
        if (b.Contains(point))
        {
            outT = t.first;
            outNormal = t.second; // AABBの各平面の法線ベクトル
            return true;
        }
    }
    
    // None of the intersections area within bounds of box
    return false;
}

// フレーム間での球と球の衝突検知
bool SweptSphere(/* prev frame */const Sphere& P0,
                 /* next frame */const Sphere& P1,
                 /* prev frame */const Sphere& Q0,
                 /* next frame */const Sphere& Q1,
                 float& outT)
{
    ...
    else
    {
        ...
        outT = (-b - discriminant) / (2.0f * a); // 解の公式
