...
bool AudioSystem::Initialize()
{
    ...
    // Create FMOD stdio system object
    FMOD_RESULT result;
    ...
    result = mSystem->initialize(
                                  ...
                                  FMOD_INIT_NORMAL, // Use default setting
...
void AudioSystem::LoadBank(const std::string& name)
{
    ...
    FMOD_RESULT result =
        mSystem->loadBankFile(name.c_str(), // File name of bank
    ...
    if (result == FMOD_OK)
    {
        ...
        // Load all non-streaming sample datea
        bank->loadSampleData();
...
void AudioSystem::UnloadBank(const std::string &name)
{
    ...
    // bank -> bus -> event(->Description->Instance)
...
void AudioSystem::UnloadAllBanks()
{
    ...
    {
        // Unload the sample dta, then the bank itself
        iter.second->unloadSampleData();
        iter.second->unload();
    }
    mBanks.clear();
    
    // No banks means to events
    mEvents.clear();
}
...
SoundEvent AudioSystem::PlayEvent(const std::string &name)
{
    ...
    auto iter = mEvents.find(name); // EventDescription
    if (iter != mEvents.end())
    {
...
namespace
{
    FMOD_VECTOR VecToFMOD(const Vector3& in)
    {
        ...
        // to FMOD (+z forward, +x right, +y up) direct3d style
        FMOD_VECTOR v;
        v.x = in.y;
        v.y = in.z;
        v.z = in.x;
...
void AudioSystem::SetListener(const Matrix4 &viewMatrix)
{
    ...
    // Set velocity to zero (fix if using Dopper effect)
...
AudioSystem::GetEventInstance(unsigned int id)
{
    FMOD::Studio::EventInstance* event = nullptr;
