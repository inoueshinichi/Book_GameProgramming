...
{
    // Update each component separetely
...
bool AABB::Contatins(const Vector3& point) const
{
    bool outside =
        point.x < mMin.x || /* min */
        point.y < mMin.y ||
...
float AABB::MinDistSq(const Vector3 &point) const
{
    ...
    float dx = Math::Max(mMin.x - point.x, 0.0f); // 原点に近い側をチェック
    dx = Math::Max(dx, point.x - mMax.x); // 原点から遠い側をチェック
    float dy = Math::Max(mMin.y - point.y, 0.0f); // 原点に近い側をチェック
    dy = Math::Max(dx, point.y - mMax.y); // 原点から遠い側をチェック
    float dz = Math::Max(mMin.z - point.z, 0.0f); // 原点に近い側をチェック
    dz = Math::Max(dz, point.z - mMax.z); // 原点から遠い側をチェック
...
bool Capsule::Contatins(const Vector3 &point) const
{
...
bool ConvexPolygon::Contains(const Vector2& point) const
{
    ...
    // Return true if approximately 2pi. 必ず誤差がでる.
...
{
    bool no =
        a.mMax.x < b.mMin.x || /* a ... b */
        a.mMax.y < b.mMin.y ||
        a.mMax.z < b.mMin.z ||
        b.mMax.x < a.mMin.x || /* b ... b */
        b.mMax.y < a.mMin.y ||
        b.mMax.z < a.mMin.z;
...
bool Intersect(const LineSegment& l, const Sphere& s, float& outT)
{
    ...
    else
    {
        ...
        // Cehck whether either t is within bounds of segment. thus check t[0,1]
...
{
    // First test if there's a solution for t (分母のチェック)
    float denominator = Vector3::Dot(l.mEnd - l.mStart, p.mNormal); // 分母
    ...
    {
        float numerator = -Vector3::Dot(l.mStart, p.mNormal) - p.mD; // 分子
        
        outT = numerator / denominator;
        
        // Validate if within bounds of the line segment
...
// 線分 vs AABB
bool Intersect(const LineSegment& l, const AABB& b, float& outT, Vector3& outNormal)
{
    ...
    TestSidePlane(l.mStart.x, l.mEnd.x,
                  b.mMin.x, Vector3::NegUnitX, tValues); // neg dir
    TestSidePlane(l.mStart.x, l.mEnd.x,
                  b.mMax.x, Vector3::UnitX, tValues); // pos dir
    
    // Test the y planes
    TestSidePlane(l.mStart.y, l.mEnd.y,
                  b.mMin.y, Vector3::NegUnitY, tValues); // neg dir
    TestSidePlane(l.mStart.y, l.mEnd.y,
                  b.mMax.y, Vector3::UnitY, tValues); // pos dir

    // Test the z planes
    TestSidePlane(l.mStart.z, l.mEnd.z,
                  b.mMin.z, Vector3::NegUnitZ, tValues); // neg dir
    TestSidePlane(l.mStart.z, l.mEnd.z,
                  b.mMax.z, Vector3::UnitZ, tValues); // pos dir
    ...
    // Test if the box contatins any of these points of intersection.
    Vector3 point;
    for (auto& t : tValues)
    {
        point = l.PointOnSegment(t.first);
        if (b.Contatins(point))
        {
            outT = t.first;
            outNormal = t.second;
            return true; // 最も小さいtの値で終了する
        }
    }
