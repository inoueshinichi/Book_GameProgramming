...
void FPSActor::UpdateActor(float deltaTime)
{
    ...
    Vector3 modelPos = GetPosition(); // Actorの座標系原点位置 on World座標系
    // FPSActorの(前方ベクトル,右方ベクトル,上方ベクトル)に対する射影になる?
    modelPos += GetForward() * modelOffset.x;
    modelPos += GetRight() * modelOffset.y;
    modelPos.z += modelOffset.z; // 上方ベクトルはWorld座標系の上方ベクトルと同じ.
...
{
    // Pause heare because the way I setup the parameter in FMOD
