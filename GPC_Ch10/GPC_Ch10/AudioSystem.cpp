    ...
    if (result == FMOD_OK)
    {
        ...
        if (numEvents > 0)
        {
            ...
            for (int i = 0; i < numEvents; ++i)
            {
                ...
                // Get the path of this event (link event:/Explosion2D)
...
void AudioSystem::UnloadBank(const std::string& name)
{
    ...
    // Second we need to remove all `buses` from this bank
    int numBuses = 0;
    bank->getBusCount(&numBuses);
    if (numBuses > 0)
    {
...
SoundEvent AudioSystem::PlayEvent(const std::string &name)
{
    ...
    if (iter != mEvents.end())
    {
        ...
        iter->second->createInstance(&event); // EventDescriptorからEnventInstanceを生成
        if (event)
        {
...
{
    // Find any stoped event instances
