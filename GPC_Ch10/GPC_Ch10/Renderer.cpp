...
bool Renderer::Initialize(float screenWidth,
                          float screenHeight)
{
    ...
    // Request a color bufferwith 8-bits per RGBA channel
    SDL_GL_SetAttribute(SDL_GL_RED_SIZE, 8);
    SDL_GL_SetAttribute(SDL_GL_GREEN_SIZE, 8);
    SDL_GL_SetAttribute(SDL_GL_BLUE_SIZE, 8);
    SDL_GL_SetAttribute(SDL_GL_ALPHA_SIZE, 8);
    SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE, 24); // depthは24bit
    ...
    mWindow = SDL_CreateWindow("Game Programming in C++ (Chapter 10)",
                               100,
                               100,
...
{
    // glClipControl(GL_LEFT_TOP)を使えば, NDCのY軸を反転できるが,
    // このプログラムでは行っていない.
    float vertices[] = {
...
void Renderer::SetLightUniforms(class Shader *shader)
{
    ...
    shader->SetVectorUniform("uDirLight.mDiffuseColor", mDirLight.mDirection);
...
Vector3 Renderer::Unproject(const Vector3& screenPoint) const
{
    ...
    Vector3 deviceCoor = screenPoint;
    deviceCoor.x /= (mScreenWidth) * 0.5f;
    deviceCoor.y /= (mScreenHeight) * 0.5f;
    
    // Transform vector by unprojection matrix
    Matrix4 unprojection = mView * mProjection;
    unprojection.Invert();
    return Vector3::TransformWithPerspDiv(deviceCoor, unprojection);
}
...
void Renderer::GetScreenDirection(Vector3& outStart, Vector3 &outDir) const
{
    ...
    // Get and point (in center of screen, between near and far)
    screenPoint.z = 0.9f; // NDCのZ軸は[0,+1]
