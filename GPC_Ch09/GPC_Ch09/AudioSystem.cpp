...
bool AudioSystem::Initialize()
{
    ...
    result = mSystem->initialize(512, // Max number of concurrent sounds
                                 FMOD_STUDIO_INIT_NORMAL, // Use default setttings
...
void AudioSystem::LoadBank(const std::string &name)
{
    ...
    FMOD_RESULT result = mSystem->loadBankFile(name.c_str(), // File name of bank
    ...
    if (result == FMOD_OK)
    {
        ...
        if (numEvents > 0)
        {
            // Get list of event descriptions in bank
            std::vector<FMOD::Studio::EventDescription*> events(numEvents);
            bank->getEventList(events.data(), numEvents, &numEvents); // capacity == numEvents
...
void AudioSystem::UnloadBank(const std::string& name)
{
    ...
    // First we need to remove all events(descriptions) from this bank
...
SoundEvent AudioSystem::PlayEvent(const std::string &name)
{
    ...
    if (iter != mEvents.end())
    {
        ...
        iter->second->createInstance(&event); // EventDiscriptionがEventInstanceを作る
        if (event)
        {
    ...
    // SoundEventクラスにretIDをパックしてSoundEventを作る(Factory)
...
void AudioSystem::SetListener(const Matrix4 &viewMatrix)
{
    ...
    // In the inverted view, third row is forward (列優先表現ならば, 3列目の列ベクトルz)
    listener.forward = VecToFMOD(invView.GetZAxis());
    // In the inverted view, secod row is up (列優先表現ならば, 2行目の列ベクトルy)
