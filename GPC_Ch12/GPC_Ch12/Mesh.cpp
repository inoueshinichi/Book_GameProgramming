...
bool Mesh::Load(const std::string &fileName, Renderer* renderer)
{
    ...
    size_t verSize = 8; // VAの長さ(x,y,z,nx,ny,nz,ux,vy) + (skin_info1, skin_info2)
    ...
    for (rapidjson::SizeType i = 0; i < vertsJson.Size(); i++)
    {
        ...
        if (!vert.IsArray())
        {
            SDL_Log("Unexpected vertex format for %s",
                    fileName.c_str());
            return false;
        }
        ...
        mRadius = Math::Max(mRadius, pos.LengthSq()); // 最大半径を更新
        mBox.UpdateMinMax(pos); // AABBの範囲を更新
    ...
    st::vector<unsigned> indices;
    indices.reserve(indJson.Size() * 3); // 3頂点
    ...
    mVertexArary = new VertexArray(
                        /*頂点配列の先頭ポインタ*/vertices.data(),
                        /*頂点数*/static_cast<unsigned>(vertices.size())/ vertSize,
                        /*レイアウト*/layout,
                        /*頂点番号配列の先頭ポインタ*/indices.data(),
                        /*頂点番号配列の長さ*/static_cast<unsigned>(indices.size())
                                   );
