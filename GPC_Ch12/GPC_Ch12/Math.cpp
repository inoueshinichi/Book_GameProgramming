...
// 2D transform
Vector2 Vector2::Transform(const Vector2& vec,
                           const Matrix3& mat,
                           float w /*= 1.0f*/)
{
    /* 行優先表現 */
...
// 3D tansform
Vector3 Vector3::Transform(const Vector3& vec,
                           const Matrix4& mat,
                           float w /*= 1.0f*/)
{
    /* 行優先表現 */
...
// 3D transform 透視投影行列変換 & w要素によるPerspective Division
                                       ...
                                       float w /*= 1.f*/)
{
   ...
   float transformedW = vec.x * mat.mat[0][3] + vec.y * mat.mat[1][3] +
    ...
    // Perspective Division (Cam +Z -> NDC +Z) if w is not Zero.
...
// 3D transform by quaternion
Vector3 Vector3::Transform(const Vector3& v,
                           const Quaternion& q)
{
    // v + 2.f * cross(q.xyz, cross(q.xyz,v) + q.w*v)
